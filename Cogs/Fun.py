...
class Fun(commands.Cog):
        ...
        if re.search(r";(.*?);", message.content):
            ...
            for i in list_of_words:
                if i.startswith(";"):
                    ...
                    emoji = process.extract(emoji_to_convert, self.bot.emojis, limit=1)[
                        0
                    ]
                    if int(emoji[1]) > 80:
    ...
    async def quote(self, ctx):
        async with self.bot.session.get("https://api.quotable.io/random") as random:
    ...
    async def bignate(self, ctx):
        ...
        async with self.bot.session.get(f"http://gocomics.com/bignate/{date}") as r:
            await ctx.trigger_typing()
            data = await r.text()
